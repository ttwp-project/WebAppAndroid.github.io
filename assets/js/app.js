...
$(document).ready(function(){
    ...
    for (var i = 0; i < 3; i++){
        // Display date
        day = $("#meteo-day-" + (i+1));
        day.find(".name").text(date.format("День недели: d/"));
        day.find(".date").text(date.format("DD/MM/YYYY/hh:mm:ss"));
        // Go to the next day
    ...
    getMeteoByCity(city, function (data, error) {
        if (error == null) {
            ...
            meteoTitle.html('Город не найден! <span class="text-muted">' + city + '</span> попробуйте набрать без ошибок и пробелов!');
        }
        // Stop loader
        setTimeout(function () {
...
$("#meteo-form").submit(function (event) {
    ...
    getMeteoByCity(city, function (data, error){
        if (error == null) {
            ...
            meteoTitle = $('#meteo-title span');
            meteoTitle.html('Город не найден! <span class="text-muted">' + city + '</span> Поробуйте набрать без ошибок и пробелов!');
        }
        // Stop loader
        setTimeout(function () {
            loading.attr('class', 'loading')
...
function displayMeteo(data){
    ...
    $('#meteo-title span').html('Погода в <a href="' + googleMapCity + '" class="text-muted meteo-city" target="_blank">' + data.city.name + ', ' + data.city.country + '</a>');
    ...
    for (var i = 0; i < 3; i++){
        ...
        wind.text(meteo.wind.speed + " м/с");
